    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            replace("${aws_eks_cluster.clustermercadolibre-case.identity.0.oidc.0.issuer}:sub", "https://", "") = "system:serviceaccount:default:irsasecrets"
          }
        }
      },
    ]
...
resource "aws_eks_cluster" "clustermercadolibre-case" {
  name     = "mercadolibre-case"
...
resource "aws_eks_node_group" "appNodeGroupmercadolibre-case" {
  cluster_name    = aws_eks_cluster.clustermercadolibre-case.name
  node_group_name = "appNodemercadolibre-case"
...
data "tls_certificate" "thumbprint_eks" {
  url = aws_eks_cluster.clustermercadolibre-case.identity.0.oidc.0.issuer
}
